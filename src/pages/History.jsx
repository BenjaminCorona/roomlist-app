...
import { DateTime } from 'luxon'; // Asegúrate de haber instalado luxon
  ...
  const loadActivityData = async () => {
    try {
      ...
      const formattedData = records.map((record) => {
        const date = record.Fecha_Cambio.replace(' ', 'T');
        const date2 = date.replace('Z', '');
        const localDate = DateTime.fromISO(date2, { zone: 'America/Argentina/Buenos_Aires' });
        const formattedDate = localDate.toFormat('dd/MM/yyyy HH:mm:ss');
        return {
          id: record.id,
          Descripcion_Cambio: record.Descripcion_Cambio,
          Fecha_Cambio: formattedDate,
          expand: {
            sala: {
